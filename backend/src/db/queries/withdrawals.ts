...
export class WithdrawalQueries {
    /**
    ...
    /**
     * Find withdrawals by wallet address
     */
     ...
     */
    static async findByTxHash(txHash: string): Promise<Withdrawal | null> {
        const query = `
            SELECT * FROM withdrawals
            WHERE tx_hash = $1
        `;
    ...
    /**
     * Map database row to Withdrawal interface
     */
    private static mapRow(row: any): Withdrawal {
        return {
            id: row.id,
            userId: row.user_id,
